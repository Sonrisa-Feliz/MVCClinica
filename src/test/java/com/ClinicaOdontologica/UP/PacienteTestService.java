...
public class PacienteTestService {
    ...
    void guardarPaciente_PersisteYRetornar() {
        ...
        assertTrue(buscado.isPresent(), "El paciente debería existir en la base de datos");
        assertEquals(guardado.getId(), buscado.get().getId(), "El id del paciente guardado es incorrecto");
        assertEquals(guardado.getNombre(), buscado.get().getNombre(), "El nombre del paciente guardado es incorrecto");
    }
    ...
    void buscarPorEmail_deberiaEncontrarPaciente() {
        ...
        // ENTONCES
        assertTrue(resultado.isPresent(), "Debería encontrar un paciente con ese email");
    ...
    void borrarPaciente_deberiaEliminarPaciente() {
        // DADO
        System.out.println("Prueba eliminación de Paciente");
        Paciente paciente = crearPacienteTestUnitario("borrar@up.edu");
        Paciente guardado = pacienteService.guardarPaciente(paciente);
        Long id = guardado.getId();
        assertNotNull(id, "El id del paciente de prueba no debería ser nulo");
        ...
        assertTrue(eliminado.isEmpty(), "El paciente debería haber sido eliminado");
    }

    @Test
    void modificarPaciente_deberiaModificarPaciente() {
        // DADO
        System.out.println("Prueba modificación Paciente");
